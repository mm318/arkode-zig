...
fn extractIncludeDirsFromCompileStepInner(
    ...
    lazy_path_output: *std.ArrayList(std.Build.LazyPath),
) void {
    for (step.root_module.include_dirs.items) |include_dir| {
        switch (include_dir) {
            .other_step => |other_step| {
                if (!std.mem.eql(u8, other_step.name, "gmock") and !std.mem.eql(u8, other_step.name, "gtest")) {
            ...
            .config_header_step => |config_step| {
                lazy_path_output.append(b.allocator, config_step.getOutputDir()) catch @panic("OOM");
            },
            .framework_path => |path| {
...
fn makeCdb(step: *std.Build.Step, make_options: std.Build.Step.MakeOptions) anyerror!void {
    ...
    const global_cache_root = b.graph.global_cache_root.path orelse b.cache_root.path orelse (try std.Io.Dir.cwd().realPathFileAlloc(io, ".", allocator));
    ...
    var file = try cwd.createFile(io, "compile_commands.json", .{});
    defer file.close(io);

    const cwd_string = try dirToString(allocator, io, cwd);
...
fn writeCompileCommands(io: std.Io, file: *std.Io.File, compile_commands: []CompileCommandEntry) !void {
...
fn dirToString(allocator: std.mem.Allocator, io: std.Io, dir: std.Io.Dir) ![]const u8 {
    return dir.realPathFileAlloc(io, ".", allocator) catch |err| {
